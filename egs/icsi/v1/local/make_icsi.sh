...
find "${ICSI_CORPUS_DIR}/NIST_Trans" -name "*.xml" | while read xml_file; do
  # Extract meeting ID from the XML file path (e.g., Bmr001)
  meeting_id=$(basename "${xml_file}" .xml)

  # Construct the path to the corresponding audio file (.sph)
  audio_file="${ICSI_CORPUS_DIR}/audio/${meeting_id}.sph"

  if [ ! -f "${audio_file}" ]; then
    echo "Warning: Audio file not found for ${meeting_id}: ${audio_file}. Skipping."
    continue
  fi
  ...
  python3 local/icsi_to_rttm.py "${xml_file}" "${meeting_id}" \
    1>> "${OUTPUT_DATA_DIR}/segments_temp" \
    2>> "${OUTPUT_DATA_DIR}/utt2spk_temp" \
    3>> "${OUTPUT_DATA_DIR}/rttm_temp" || { echo "Error running icsi_to_rttm.py for ${meeting_id}"; exit 1; }
...
utils/validate_data_dir.sh --no-feats "${OUTPUT_DATA_DIR}" || exit 1; # --no-feats because features are made in run.sh
